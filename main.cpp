...
void CreateDummyMovieAndEncode(char A[20], char Dummy[20])
{
    ...
    for (; I<20 && A[I]!='\0'; I++)
    {
        if (A[I]=='A' || A[I]=='E' || A[I]=='I' || A[I]=='O' || A[I]=='U') Dummy[I]=A[I]; //ONLY KEEP VOWEL CHARACTERS VISIBLE
        else if (A[I]==' ') Dummy[I]=' '; //KEEP SPACE AS A SPACE CHARACTER ONLY
        else Dummy[I]='*'; //REPLACE NON-VOWEL CHARACTERS WITH AN ASTERIX
    }
...
void FillOrSlash(char Input, char A[20], char Dummy[20])
{
    ...
    do{
        ...
        if(strcmp(A,Dummy)==0) //IMPLIES THE USER HAS CORRECTLY GUESSED THE WHOLE MOVIE
        {
            ...
            cout<<Dummy<<endl<<"CONGRATULATIONS, YOU GUESSED THE MOVIE RIGHT!!"<<endl;
        }
        else if(!Found) //IMPLIES USER'S GUESSED CHARACTER WAS NOT PRESENT
        { 
            Chances--; //DECREMENT CHANCES FOR INCORRECT GUESSES
            cout<<"Character not present!"<<endl<<Dummy<<endl;
            cout<<"Chances Remaining: "<<Chances<<endl; 
        }
        else 
            cout<<"Good guess!"<<endl<<Dummy<<endl; //IMPLIES USER GUESSED CORRECTLY
